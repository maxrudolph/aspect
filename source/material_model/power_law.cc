	  ...
	  if( eta_eff < etamin ){
	    ...
	    eta_eff = etamax;
          ...
          out.viscosities[i] = ((composition_viscosity_prefactor != 1.0) && (in.composition[i].size()>0))
                               ...
                               pow(10.0, ((1-in.composition[i][0]) * log10(eta_eff*temperature_dependence)
                                          + in.composition[i][0] * log10(eta_eff*composition_viscosity_prefactor*temperature_dependence)))
                               :
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(PowerLaw,
                                   "power-law",
